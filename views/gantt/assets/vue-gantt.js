...
], function(ganttCore, ganttTmpl, ganttBarTmpl) {
  var dayMillSeconds = 24 * 60 * 60 * 1000;

    Vue.component('gantt', {
        template: ganttTmpl,
        props: ['type', 'list', 'setHeight', 'assetsDateRange'],
        data: function () {
            return {
                ...
                rangeDate: ganttCore.getDateRange(this.assetsDateRange, this.type),
                //rangeDate: ganttCore.getDateRange(this.list, this.type),
                vLeftWidth:(localStorage && localStorage.getItem('progressGantt.LeftWidth')) || "450"
            };
        },
        methods: {
            ...
            getEllipsDays: function (planStartDate, planEndDate) {
                if (planStartDate && planEndDate) {
                    var ellipsMillSeconds = ganttCore.dateHelper.getDate(planEndDate) - ganttCore.dateHelper.getDate(planStartDate);
            ...
            autoScrollToNow: function () {
                ...
                if (currentDayIndex) {
                    //这里滚动可能会存在差了几个像素的情况
                    $scrollTarget.stop().animate({ scrollLeft: (currentDayIndex * $currentDay.width()) - $scrollTarget.width() / 2 }, 800, 'linear');
                }
            },
            initHover: function () {
               
                $(this.$el).on('mouseover.hover', '.gantt-body-container>.gantt-row', function (e) {
                    var $target = $(e.currentTarget);
                    var id = $target.data('id');
                    $('.gantt-body-container>.gantt-row[data-id="' + id + '"]').addClass('hover');
                }).on('mouseout.hover', '.gantt-body-container>.gantt-row', function (e) {
                    var $target = $(e.currentTarget);
                    var id = $target.data('id');
                    $('.gantt-body-container>.gantt-row[data-id="' + id + '"]').removeClass('hover');
                })
            }
        },
        mounted: function () {
        ...
        watch: {
            type: function (val) {
                this.rangeDate = ganttCore.getDateRange(this.assetsDateRange, val);
            },
            assetsDateRange: function (values) {
                this.rangeDate = ganttCore.getDateRange(values, this.type);
            }
        },
        computed: {
